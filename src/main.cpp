...
#if defined(NDEBUG)
#error "Intercash cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uitc) are considered zero fee (for relaying and mining)
...
{
    RenameThread("Intercash-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ITC: We use certain sporks during IBD, so check to see if they are
